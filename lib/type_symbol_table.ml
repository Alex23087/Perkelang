...
let builtin_types =
  ...
  let rem_ptr s =
    ...
    if len < 4 then
      ...
      String.sub s 0 (len - 4) (* maybe should check the string is "_ptr" *)
  
let rec is_builtin_type k (typ, code) = match typ with
  | _, Pointertype typ1, _ -> let k = rem_ptr k in is_builtin_type k (typ1, code)
  | _ -> List.mem (k, (typ, code)) builtin_types

let rec is_builtin_type_unlabeled typ = match typ with
...
let rec c_type_of_base_type (t : perktype) : string = match t with
...
and c_type_of_perktype ?(erase_env = true) (t : perktype) = 
  if is_builtin_type_unlabeled t then c_type_of_base_type t else type_descriptor_of_perktype ~erase_env t
