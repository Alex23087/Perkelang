...
let fresh_var (): string =
  ...
  Printf.sprintf "__perkelang_lambda%d" v
...
and get_lambda (e: expr) : (string * string) =
  ...
  with Not_found ->
    ...
    | Lambda (retype, args, body) -> (
      ...
      let body_str = codegen_command body in
...
and codegen_program (cmd: command) : string =
  let body = codegen_command cmd in
...
and codegen_command (cmd: command) : string =
  match cmd with
  | InlineC s -> s
  | Block c -> codegen_command c
  | Def (t, e) -> codegen_def t e
  | Fundef (Fun(t, id, args, body)) -> codegen_fundef t id args body
  | Assign (s, e) -> Printf.sprintf "%s = %s;" s (codegen_expr e)
  | Seq (c1, c2) -> Printf.sprintf "%s\n%s" (codegen_command c1) (codegen_command c2)
  | IfThenElse (e, c1, c2) ->
      Printf.sprintf "if (%s) {\n%s\n} else {\n%s\n}" (codegen_expr e) (codegen_command c1) (codegen_command c2)
  | Whiledo (e, c) ->
      Printf.sprintf "while (%s) {\n%s\n}" (codegen_expr e) (codegen_command c)
  | Dowhile (e, c) ->
      Printf.sprintf "do {\n%s\n} while (%s);" (codegen_command c) (codegen_expr e)
  | For (e1, e2, c1, c2) ->
      Printf.sprintf "for (%s; %s; %s) {\n%s\n}" (codegen_expr e1) (codegen_expr e2) (codegen_command c1) (codegen_command c2)
  | Expr e -> Printf.sprintf "%s;" (codegen_expr e)
  | Skip -> ""
  | Switch (e, cases) ->
      let cases_str = String.concat "\n" (List.map (fun (e, c) -> Printf.sprintf "case %s: {\n%s\n}" (codegen_expr e) (codegen_command c)) cases) in
      Printf.sprintf "switch (%s) {\n%s\n}" (codegen_expr e) cases_str
  | Return e -> Printf.sprintf "return %s;" (codegen_expr e)
